...
- **`core/agency/`** — Real-world capability tools. Now includes `documents.py` (PDF via pypdf & text document understanding, line-by-line keyword searching), `email_gmail.py` (Gmail API integration for checking inbox, search, draft, send, with graceful degradation when unconfigured), and `seelsupport.py` (Laravel Sanctum + InfinityFree AES cookie challenge bypass for full CRUD operations and proactive notification polling). Real-world tools are assembled by `registry.py` (schemas + permission-gated `dispatch`) and called via `brain.execute_tool`. New permission categories include `schedule`, `documents`, `email`, and `seelsupport` (default allowed). Refuses catastrophic commands.
